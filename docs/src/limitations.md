...
- **The onset curve is read from a figure.** Each bar is digitised from a figure that each report redraws, so the counts carry scan error that the model treats as noise at a fixed scale.
...
- **Streams share one case pool.** They are fitted as conditionally independent given latent incidence but observe overlapping people.
  This can understate uncertainty, which the [outbreak size estimated by each data stream](@ref "Outbreak size estimated by each data stream") checks.
...
- **The bed shortfall is not measured.** Occupancy shows demand only up to the beds filled, so the shortfall above capacity comes from the [treatment-centre flow](@ref "Treatment-centre flow") model and its priors.
...
- **Reconstructed forecasts are not exact.** Past forecasts that were not stored are rebuilt from each release's own code, but with current dependency versions.

## Provinces

### Data

- **Province data are confirmed cases and deaths only.** Occupancy and beds by province are not fitted, and there is no province onset nowcast.
...
- **Four patches, not the full provincial detail.** Ituri, Nord-Kivu and Haut-Uele are modelled individually and every other affected province is pooled into a fourth patch.
...
- **Provincial testing enters the prior, not the likelihood.** The deaths do most of the work in separating a province's incidence from its case-finding, and the prior strongly influences the rest ([province parameters](@ref "Province parameters against their priors")).
