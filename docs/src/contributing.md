...
- `exponential_growth_model` samples the growth rate `r` and the generation count `m`, not `r` and `T` directly, to break the `C(T) = exp(rT)` ridge. The cryptic duration is `T = m·G` with `G` the mean generation interval.
