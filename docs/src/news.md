...
- The provincial Rt deviations start from the stationary distribution of their mean-reverting process, `σ_level = σ_drift / √(1 - φ²)`, rather than at a sampled scale `σ_level ~ half-N(0, 0.15)`.
  The sampled scale spread three deviations and formed a funnel with Nord-Kivu's starting deviation (correlation 0.71).
  The chain key `σ_level` is removed; `region_sd` reports the derived scale.
