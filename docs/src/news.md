...
At the old `truncated(Normal(5, 4); lower = 0)` the 99th percentile is 22,722 infections per day — against a fitted outbreak of about 12,700 infections in total (`data/size_by_release_by_stream.csv`, results-v1.16.0), so a prior on *one day* reaches nearly twice the whole outbreak.
