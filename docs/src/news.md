...
- The Provinces page opens with a summary of each province's infections, reproduction number, case-fatality ratio and relative case ascertainment as 90% credible intervals, one bullet per province (#832).
  The per-province table of 30%, 60% and 90% intervals sits behind a dropdown beneath it, and the point-estimate overview table is gone.
  Pair plots set the spatial hyperparameters and each province's parameters against a prior drawn from the patch model.
- The by-province tables move from the National page's summary to the Provinces page, and the summary dashboard's by-province block reads the same bullets (#832).
