...
# \qquad
# m_0 = 6 + \frac{\text{cut-off} - \text{18 May 2026}}{20}. \tag{3}
# ```
#
# The base anchors the outbreak *start* (seeding time), not its size.
# The implied start is $T = m\cdot\tau$ with $\tau = \log 2/r$, so the
# base must rescale whenever the growth prior's central doubling time
# changes, to hold the same seeding time. McCabe et al.'s first report
# (18 May 2026) implied $m = 9$ at their 14-day doubling, a start
# $T = 9\cdot 14 = 126$ days before 18 May. Holding that start under the
# molecular-clock 20-day doubling
# (the [cuomodannenburg2026](@cite) estimate, shared with the
# growth-rate prior above) rescales the base to $9\cdot 14/20 \approx
# 6.3$, rounded to $6$. This implies a small BVD outbreak at 18 May
# ($2^6 \approx 64$ BVD), with the bulk of the observed suspected cases
# being non-BVD background, coherent with the high-background picture.
# Each day that the cut-off runs past that report adds a fraction of a
# doubling at the same 20-day doubling time, so the prior stays anchored
# on the plausible start as the data are refreshed rather than being
# fixed at the report-date value.
