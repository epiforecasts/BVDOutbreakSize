...
# and figure below are replaced by a short note in place of the re-fit.
#
# The confirmed stream scales true BVD infections by the confirmation
# sensitivity. The headline model credits the repeat-control confirmation
# process (mean 0.95), because the situation reports show rule-out is an
# investigative decision, not one negative PCR: suspects are confirmed or
# reclassified through a deep investigation with repeat control tests
# (discharge itself needs two negatives), a probable category absorbs untested
# deaths, and the non-cas label is reversible [see issue #374]. Two independent
# control tests lift the effective sensitivity to about
# $1-(1-0.85)^2\approx0.98$, so the single analytical assay draw (mean 0.85)
# understates it.
# This variant re-fits the joint model with that single-assay prior on the
# confirmed stream as the downside check and compares the infection count to
# date. The re-fit uses the full headline settings (1000 draws across two
# chains).
#
# A lower sensitivity means each confirmed case implies more true infections,
# so the infection count should if anything edge up; a large shift would flag
# that the confirmation-sensitivity assumption matters for the estimate.

#md # ```@raw html
#md # <details><summary>Re-fit the joint with the single-assay downside sensitivity</summary>
#md # ```

## The rule-out (single-assay downside) re-fit is defined in the fit registry
...
ruleout_sensitivity_table = RUN_SENSITIVITY ?
                            streams_table(
    "headline (repeat-control process s)" => posterior_C_joint,
    "single-assay downside s" => posterior_C_ruleout) :
...
ruleout_sensitivity_fig = RUN_SENSITIVITY ?
                          plot_cumulative_cases(
    "headline (repeat-control process s)" => posterior_C_joint,
    "single-assay downside s" => posterior_C_ruleout; scenarios = []) :
                          Markdown.md"_Rule-out sensitivity runs on main and release builds only; skipped on this build._"
