...
# The origin therefore sits $T_{\text{cryptic}} = m\,G$ days before the renewal start, with $G$ the mean generation interval, and the cryptic phase grows from the single index infection to a daily incidence of $C_T = r\,e^{r T_{\text{cryptic}}}$ at the renewal start, the day the renewal takes over.
