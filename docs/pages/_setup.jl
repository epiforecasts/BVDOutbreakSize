...
if !@isdefined(_BVD_SETUP_LOADED)
    ...
    function validation_forecast_from(frozen)
        ...
        report_days = frozen.o.onset_curve_history.report_days
        ...
        ## The calendar walk's own origin, which may run later than the
        ## first onset date `alpha` is indexed from.
        hazard_start = isnothing(grid_start) ? nothing :
            onset_hazard_grid_start(onset_days, report_days)
        return forecast_reported(
            ...
            grid_n = frozen.o.n,
            onset_grid_start = hazard_start, onset_grid_end = grid_end,
            onset_alpha_grid_start = grid_start
        )
    end
