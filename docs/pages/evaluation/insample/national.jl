...
# The band is the model's posterior predictive of each onset date's eventual reported total, sampled through the onset stream's likelihood so it carries the fitted per-snapshot noise scale, the per-figure level error and the Student-t tail.
...
## predictive of the eventual reported total. `v_idx` is the snapshot's
## position in `_onset_report_grid_days`, the index its `scan_level` and
## `τ` are stored under.
