...
const KNOWN_MIRROR_ERRORS = Dict(
    (Date("2026-08-08"), "confirmed cases") => (
        4294, 4209,
    ...
    (Date("2026-08-08"), "confirmed deaths") => (
        1960, 1916,
        "same misdated SitRep 085 row; SitRep 086's headline and province " *
            "sum both give 1960",
    ),
    (Date("2026-08-11"), "confirmed cases") => (
        4567, 4566,
    ...
    (Date("2026-08-25"), "confirmed deaths") => (
        2744, 2755,
...
function upstream_series(file)
    df = CSV.read(
        Downloads.download("$BASE_URL/$file"),
...
for (file, col, label) in SERIES
    ...
    for row in eachrow(mismatched)
        ...
        if known !== nothing && known[1] == row.scanned &&
            ...
            println(
                "  known mirror error $(row.date): scan " *
                    "$(row.scanned) vs mirror $(row.upstream) - $(known[3])"
            )
        else
            unexplained += 1
            println(
                "  MISMATCH $(row.date): scanned $(row.scanned) vs " *
                    "mirror $(row.upstream)"
            )
        end
    end
    ...
    println(
        "  $(nrow(shared)) dates compared, $(nrow(mismatched)) " *
    ...
    if nrow(new_dates) > 0
        println(
            "  mirror dates not scanned: " *
                join(
                ("$(r.date)=$(r.upstream)" for r in eachrow(new_dates)),
                ", "
            )
        )
    end
    ...
    if only_scanned > 0
        println(
            "  $only_scanned scanned date(s) the mirror does " *
...
if any_mismatch
    println(
        "Reconciliation FAILED: an undocumented disagreement, or a " *
            "stale entry."
    )
    exit(1)
else
    println(
        "Reconciliation OK: every disagreement is a documented " *
            "mirror error."
    )
end
