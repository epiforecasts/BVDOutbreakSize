...
## The tree-depth cap of the real-data fit, `fit_zone`'s default unless
## `--depth` overrides it.
const MAX_DEPTH = parse(Int, OPTS["depth"])
...
function stage_b()
    ...
    ## The walking set is data-dependent. The refit's own inputs say which
    ## zones carry a walk.
...
function stage_c(chn)
    ...
    body = """
    <p>NUTS, Mooncake, diagonal metric, target acceptance 0.85, tree depth cap
    $(MAX_DEPTH). A zone whose reproduction number is undefined in some draw
    (below the reporting floor) shows no R-hat for it.</p>
...
function stage_e(chn)
    ...
    body = """
    <p>Each zone draw carries its own draw of the patch trajectory, so the
    reproduction number's interval carries the patch uncertainty; a
    level-only zone's is its patch's scaled by a prior-driven level.</p>
    ...
    rt = [
        filter(isfinite, m[:, INPUTS.n])
            for m in reconstruct_zone_rt(chn, INPUTS)
    ]
