...
function check_mirror_lead()
    ...
    return if mirror_date > as_of
        println("\n", "!"^70)
        println(
            "WARNING: the INRB-UMIE mirror is AHEAD of the manifest ",
            "even though insp.cd has no new SitRep."
        )
        println("  Manifest as_of_date:       ", as_of)
        println(
            "  Mirror latest cumulative:  ", mirror_date, " = ",
            mirror_value, " confirmed cases"
        )
        println(
            "This does NOT mean a SitRep was missed - insp.cd was ",
...
function main()
    ...
    missing_reports = sort(
        [p for p in pubs if first(p) > recorded];
        by = first
    )

    println(
        "Latest recorded (data/insp_sitrep_scanned.csv): SitRep ",
        lpad(recorded, 3, '0')
    )
    println(
        "Latest published (insp.cd):                     SitRep ",
        lpad(latest, 3, '0')
    )
