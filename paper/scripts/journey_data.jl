...
function classify_commit(name, email)
    ...
    name in ("Sam Abbott", "Sebastian Funk", "Samuel Brand",
             "Kath Sherratt", "Katharine Sherratt") && return :human
    return :other
end
...
mainref = isempty(git("branch", "-r", "--list", "origin/main")) ?
    "main" : "origin/main"
commit_rows = map(splitlines(git("log", "--first-parent",
        "--format=%H%x09%as%x09%an%x09%ae", mainref))) do l
    h, d, n, e = split(l, '\t')
...
pr_rows = map(splitlines(gh("pr", "list", "--state", "merged",
        "--limit", "2000", "--json", "number,mergedAt,author",
        "--jq", ".[] | [.number, .mergedAt, .author.login] | @tsv"))) do l
    n, t, a = split(l, '\t')
...
write_csv("commits_weekly.csv",
    ...
    [(w, count_kind(commit_rows, w, :human),
      ...
      count_kind(pr_rows, w, :other)) for w in weeks])
println("  commits on ", mainref, ": ", length(commit_rows),
...
tags = filter(t -> occursin(r"^[vV]\d+\.\d+\.\d+$", t),
...
loc(t, files, pat) = sum(countlines_at(t, f) for f in files if occursin(pat, f);
    init = 0)

const V12_OBS = ["exports_model", "deaths_model", "cases_model",
    "exports_deaths_model", "exports_detection_timing_model"]
model_names(src) = [m.captures[1] for m in
...
function joint_body(src)
    ...
    join(filter(l -> !startswith(strip(l), "#"), lines[i:j]), "\n")
end
...
rel_lines = splitlines(read(joinpath(REPO, "data", "released_estimates.csv"),
    String))
...
write_csv("fit_cost.csv",
    ["date", "tag_or_pr", "arm", "model", "joint_gradient_ms",
     "joint_fit_minutes", "source_url", "note"], fit_cost)
