...
const ASSETS = ("imperial_comparison.csv", "scenario_coverage.csv",
...
axkw = (; xgridvisible = false, ygridvisible = false,
    ...
    ytickwidth = 0.6)
panel_letter(pos, s) = Label(pos, s; font = :bold, fontsize = 9,
...
function interval!(ax, y, lo_out, hi_out, lo_in, hi_in, mid, colour)
    ...
    mid === nothing && return
    scatter!(ax, [mid], [y]; color = colour, markersize = 6,
...
m1_points = [num(r.Reported_cases) for r in scenarios
             if startswith(r.Scenario, "Method 1")]
m2_points = [num(r.Reported_cases) for r in scenarios
...
for (y, (key, _, obskey)) in zip(yb, stream_rows)
    ...
    lines!(axb, [num(r.Lower_90_), num(r.Upper_90_)], [y, y];
        color = colour, linewidth = 0.8)
    lines!(axb, [num(r.Lower_60_), num(r.Upper_60_)], [y, y];
        color = colour, linewidth = 1.8)
    lines!(axb, [num(r.Lower_30_), num(r.Upper_30_)], [y, y];
...
qs = [quantile(view(Cmat, k, :), p) for k in axes(Cmat, 1),
...
axc = Axis(fig[2, 1:2]; axkw..., ylabel = "Cumulative infections",
...
for (key, label, dy) in (("reported_cases", "suspected cases", 0),
                         ("total_deaths", "suspected deaths", 0))
    scatter!(axc, [0.0], [obs[key]]; color = C_OBS, markersize = 6,
        strokecolor = :white, strokewidth = 0.6)
    text!(axc, -4, obs[key] + dy; text = label, fontsize = FS_SMALL,
        color = C_OBS, align = (:right, :center))
end
text!(axc, -4, 2550; text = "data cut-off\n$(Dates.format(CUTOFF, "d U yyyy"))",
...
axs = Axis(fig[3, 1:2]; axkw..., xlabel = "Date",
...
density!(axs, seed_days; color = (C_JOINT, 0.35), strokecolor = C_JOINT,
...
println("draws: ", length(C_draws), "  median C_T: ", round(Int, q(0.5)),
    "  50%: ", round.(Int, (q(0.25), q(0.75))),
    "  90%: ", round.(Int, (q(0.05), q(0.95))))
seed_q = [CUTOFF - Day(round(Int, quantile(T_draws, p)))
