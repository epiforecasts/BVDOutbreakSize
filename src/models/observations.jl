...
process that did not exist before `start`. Gates the laboratory
analysed-specimen volume before testing began, so it does not accrue over
the pre-surveillance cryptic phase. The suspected-case and suspected-death
streams are not gated, since those counts did accumulate over that phase.
...
and the grid length `n` (day `n` is the cut-off). A non-empty history
already ends at the cut-off, so the increments are differenced from it
alone and `total` is not scored again. When the history is empty but a
`total` is supplied (the tests-analysed stream, which has no dated vintage
history), the cut-off becomes a single vintage point at day `n`. A history
with `days` but empty `counts` keeps the vintage day grid and leaves the
increments `missing`, the generator path `predict` resamples. When both are
empty or `total` is `missing`, the increments are `missing` over zero days.
Returns `(; days, obs_increments)` with `obs_increments` either an `Int`
vector or `missing`.
"""
...
"""
Per-vintage increment likelihood for one stream. Scores the modelled
per-vintage increments `modelled` (see [`bin_increments`](@ref)) against
...
NegativeBinomial never sits against a moving wall. Each isolation day takes
the nearest recorded implied-capacity value (`capacity_history`), floored
at that day's observed occupancy so the bound is never below the count (a
count above the cap has zero probability under the censored NB). With the
cap fixed the censored likelihood is smooth in the sampled demand. A
`capacity_history` with no counts (empty, or days-only as the predictive
generator supplies) returns a large finite cap far above any bed count, so
the censoring is a no-op (a literal `Inf` cannot be used because
[`safe_rate`](@ref) maps non-finite values to `eps`). A `missing`
`iso_obs` skips the occupancy floor.

The latent bed demand is left uncensored, so demand above capacity is
carried by the renewal and length-of-stay model rather than by the bound.
Occupancy can only say demand was at least the beds filled, so the bed
shortfall is a model-informed quantity. See [`treatment_flow_model`](@ref).
"""
function censoring_cap(iso_days, iso_obs, capacity_history)
    ...
    nocap = 1.0e6
    ## Guard on the counts, not the days: the predictive generator passes the
    ## capacity history days-only, and the nearest-day lookup below would
    ## then index an empty counts vector.
...
fitted break steps `b` on the manually specified `break_days` (grid
day-indices). `Δ(t)` sums `b_j` over the break days at or before
`iso_days[t]`, zero before the first break day. Added to the modelled
occupancy mean in the censored-occupancy likelihood so the modelled
occupancy tracks a between-report measurement-basis discontinuity in the
observed series (a Tableau 6-sum to page-1 headline transition, say). Each
`b_j` is sampled in [`treatment_flow_model`](@ref), so the fit partitions
each step into reporting artefact and real demand change. Returns a
length-`length(iso_days)` vector of `eltype(b)`, zeros when there are no
break days.
"""
...
itself attributes to integrating a harmonised provincial base rather than
to the day's own notifications, with `gross` the printed 24h new-confirmed
count. On 22 July 2026 that is `369 − 97 = 272` cases and `236 − 62 = 174`
deaths. Centring on the published discrepancy rather than zero is what
makes the step identifiable from a single observation. The sampled
deviation around the centre then carries only the residual uncertainty
about how much of the discrepancy is truly retrospective.

Break days not matching a window are dropped so no inert step is sampled.
A missing or short `gross` is treated as zero, so the centre becomes the
whole increment and the entire vintage step is attributed to the artefact.
That is conservative rather than neutral, since the de-anchor drops the
day's positivity denominator and a step centred near zero over a
de-anchored day leaves nothing to absorb the backlog. Supply the printed
count so the split is data-derived.

`increments` may itself be `missing`, the generator path where no
cumulative counts are supplied to difference. The day still gets a step, so
a predictive keeps the fitted chain's dimensions, but there is no published
discrepancy to centre it on and the centre falls back to zero. Returns
`(days, centres)`, both possibly empty.
"""
...
Per-late-window confirmed harmonisation offset from the fitted break steps
`b` on the manually specified `break_days` (grid day-indices). Unlike
...
integration inflates exactly one increment and every later vintage
differences against the new, higher base. Each entry is therefore the step
for that window alone, zero on every non-break window.

Added to the modelled confirmed mean of the break window in
[`confirmed_cases_model`](@ref) and [`confirmed_deaths_model`](@ref), so
the fit can explain an increment that is mostly reattached backlog without
inflating the latent incidence that drives `Rt`. Each `b_j` is sampled
there around the published discrepancy ([`break_step_centres`](@ref)),
symmetric because a harmonisation can revise down as well as up. Returns a
length-`length(late_days)` vector of `eltype(b)`, zeros when there are no
break days.
"""
...
the death ascertainment `p_death`, and fits the between-vintage increments
with a NegativeBinomial sharing the surveillance dispersion `k`
...
separately. The onset-to-death prior is the convolution of the two atomic
line-list components (onset-to-admission and admission-to-death, from the
`bdbv-linelist-analysis` submodule), implied mean ≈ 12.8 d, SD ≈ 7.0 d.

A fatal BVD infection is counted as a suspected death only when
ascertained, the death analogue of the suspected-case ascertainment `p_drc`
(see [`death_ascertainment_model`](@ref)). Its prior is informative and
high (centre ≈ 0.9), since a death is more reliably reported than a living
suspect.

Non-BVD background suspected deaths are added on top. The joint passes the
per-day non-BVD suspected-case background `case_bg_daily` and a background
CFR submodel ([`background_cfr_model`](@ref)), so the background deaths are
...
the modelled daily suspected-death series at each report day. It continues
the suspected-death signal where the cumulative `deaths_history` stops, and
its days fall strictly after that series ends, so the two likelihoods cover
disjoint days. Empty by default.
...
("nouveaux cas suspects du jour"), per-day counts scored against the
modelled daily suspected series at each report day. It continues the
suspected signal where the cumulative `reported_history` stops, once INSP
began reclassifying suspects downward and the cumulative total fell. It
shares the suspect pipeline and dispersion with the cumulative stream, and
its days fall strictly after that series ends, so the two likelihoods cover
disjoint days. Empty by default.
...
"""
Per-window confirmed-positives likelihood. Given the per-window analysed
counts `analysed`, positivities `p_pos` and the intra-window
overdispersion `ρ`, scores the observed `positives` with one
`BetaBinomial(analysed[i], p_pos[i], ρ)` per window (see
[`safe_betabinomial`](@ref)). `ρ → 0` recovers the plain
`Binomial(analysed[i], p_pos[i])`.

`positives` is a model argument on the left of `~`, so a supplied vector is
observed data DynamicPPL conditions on and a `missing` argument is sampled
(the predictive-generator path). Under a prefixed attachment the predict
keys are `<prefix>.positives[i]`.
"""
...
the days after the last cumulative laboratory date. A day with a published
24h analysed denominator (`analysed[i] > 0`) is an overdispersed
`BetaBinomial(analysed[i], p_pos[i], ρ)` of that denominator (see
[`safe_betabinomial`](@ref)); an unanchored day is a `NegativeBinomial` of
the modelled laboratory volume `modelled[i]` sharing the surveillance
dispersion `k`. Keeping both in one submodel gives a single per-window
predict-key vector (`<prefix>.increments[i]`) over all late days in order,
so the posterior-predictive trajectory reconstructs without interleaving.
...
- *Late* windows, the confirmed vintages strictly after the last
  laboratory date (INSP's confirmed-only format, with no national
  analysed-specimen denominators). Like the early windows, their confirmed
  increments are scored against the modelled laboratory volume with the
  pooled positivity. Their day grid carries a `late_start` day (the last
  laboratory day) so the model bins the modelled volume over each late
  window's own day range, pinned there rather than from day 0, which would
  double-count the observed window volume. A late day that publishes a 24h
  analysed count (`lab_daily_history`) is flagged in `late_analysed` so the
  model can score it as a Binomial of that observed denominator, anchoring
  its positivity to data. A day listed in `confirmed_break_days` is the
  exception: its denominator is dropped (`late_analysed` set to 0) so it
  stays a modelled-volume window. On such a day INSP has retrospectively
  integrated a harmonised provincial base, so most of the confirmed
  increment is reattached backlog rather than positives out of that day's
  specimens, and pairing the two would score a wildly inflated positivity
  (22 July 2026: 369 of 414 analysed, 89% implied against the 23% actually
  reported). The increment is still scored, with the backlog absorbed by a
  fitted level step in [`confirmed_cases_model`](@ref) (see
  [`confirmed_break_offset`](@ref)).
...
function confirmed_positivity_windows(
    ...
    ## The first confirmed vintage is the baseline the surveillance system was
    ## at when reporting began, and is not scored. The early windows pin their
    ## modelled-volume accumulation at it, so no window's volume rolls over the
    ## pre-surveillance cryptic phase.
    ...
    ## date, which carry no analysed-specimen denominator. `bin_increments`
    ## runs its `prev` edge from 0, so the model pins it at `late_start` to
    ## avoid re-counting the observed-window volume scored above.
    ...
    ## Observed 24h analysed denominators on the late days: a published daily
    ## analysed count becomes that window's Binomial denominator, 0 leaves it
    ## a modelled-volume window. Only days strictly after the last cumulative
    ## laboratory date are anchored, since the cumulative series covers the
    ## rest. A plain loop rather than a Dict or filtered generator, so the AD
    ## backends can compile a rule for this integer bookkeeping on every Julia
    ## version.
    ...
    ## Retrospective harmonisation days are de-anchored, so the window falls
    ## to the modelled-volume NegativeBinomial branch instead of entering the
    ## BetaBinomial as same-day positives. Most of the confirmed increment on
    ## such a day is a provincial base integration, not specimens that tested
    ## positive out of that day's denominator, so pairing the two would drag
    ## `λ_bg`, the composition link and the ascertainment with it. The
    ## increment is still scored (see `confirmed_cases_model`).
...
## link. A top-level function with an explicit indexed loop, so its working
## variables are parameters rather than closure captures: a conditionally
...
- Specimens analysed. The suspected daily pipeline (`p_drc`-scaled BVD
  ...
  Its between-vintage increments are fitted against `lab_history` with a
  NegativeBinomial sharing the surveillance dispersion `k`, identifying
  `τ_test` and the delay. The same volume is the denominator proxy in the
  early and unanchored late windows below. The specimens-received series is
  not modelled: analysed is the throughput that produces confirmed cases,
  and received overshoots it by the laboratory backlog.
- Confirmed positives. The confirmed counts are scored as an overdispersed
  ...
  deaths and exports streams while the laboratory positivity tracks the
  noisy per-vintage data. The early confirmed vintages with no per-vintage
  analysed denominator (18-23 May) are scored as NegativeBinomial counts
...
The observed Uganda imports are a dated series, `export_case_days` giving
...
the per-day expected export increment `Λ(t) − Λ(t−1)` is the day-`t`
...
death side has no published analysed denominator, so the confirmed-death
increments are `NegBinomial(k)` counts of the modelled death volume, the
same modelled-volume route the early and post-lab confirmed-case windows
use.
...
Three pieces:
  ...
  - Death analysed volume. Deaths are tested out of the same laboratory as
    ...
    scaling, `v = scaling · case_analysed_daily · susp_death / susp_case`,
    with `susp_death` and `susp_case` the suspected deaths and cases carried
    to receipt by the confirmed cases' delay. The case volume carries the
    laboratory capacity onset, so the death volume inherits it. The scaling
    ([`death_testing_scaling_model`](@ref)) is the per-suspect
    testing-intensity difference between deaths and living suspects, and the
...
[`accumulate_occupancy`](@ref) needs: unthinned, the schedules account for
the whole admitted mass and the abscond flow removes it a second time.
...
dies before confirmation is a suspect death in the combined deaths flow.
...
`CFR`, since a recovered case is one that did not die, adjusted for the
confirmed population by a sampled log-odds offset (see
[`recovery_probability_model`](@ref)). A case is taken to be confirmed
before it is recorded as recovered, so the recovery follows confirmation by
the confirmation-to-recovery delay. A positive result could in principle
return after a patient has already recovered and been discharged, but
"cumul guéris" is assumed to count confirmed cases carefully recorded as
recovered.

The cumulative recovered series ends at the cut-off, so its between-vintage
increments are fitted with a NegativeBinomial whose dispersion is sampled
here rather than shared with the other streams. The convolution
right-censors recoveries that have not yet resolved by the cut-off, so a
small observed recovered count is consistent with a high eventual survival
fraction and a long recovery delay. Empty by default. A `missing` cut-off
total leaves the increments missing (the predictive-generator path).
Returns the recovery probability, the recovery-delay mean, the dispersion,
the daily recovered series and the cut-off total.
"""
...
`increments` is a positional model argument rather than a field read out of
a container inside the body, and that is load bearing. DynamicPPL decides
observe-versus-assume from whether the tilde's symbol is in the enclosing
...
Derived here rather than carried on the history, so any
`(; onset_days, report_days, prev_report_days, increments)` history works.
...
"""
    onset_report_scales(means, level_cur, level_prev, τ_cur, τ_prev;
        scan_sd = 0.0)

Per-cell observation scale for the reporting-triangle increment likelihood,
the square root of a variance built from two sources.
  ...
  - Digitisation noise, the fitted per-snapshot scale `τ`
    ([`onset_reporting_model`](@ref)'s noise-walk) each of the cell's two
    reads carries: `τ_cur` for the snapshot the cell belongs to, `τ_prev`
    for the one it corrects against, `0` for the virtual empty predecessor.
    An increment differences two independent reads, so the two variances add
    rather than one term doubling.
...
would bias towards overconfidence on cells that happen to undershoot. `τ` is
fitted rather than fixed, so this formula is correctable by the data without
a separate multiplicative slack. Pure, top-level, single indexed loop.
...
the report's symptom-onset reporting-delay section, along with the
per-snapshot coverage test for the shared scan error, which shows up only
once a snapshot's cells are summed.
"""
function onset_report_scales(
        means::AbstractVector,
        level_cur::AbstractVector,
        level_prev::AbstractVector,
        τ_cur::AbstractVector,
        τ_prev::AbstractVector;
        scan_sd::Real = 0.0
    )
    m = length(level_cur)
    T = promote_type(
        eltype(means), eltype(level_cur), eltype(level_prev),
        eltype(τ_cur), eltype(τ_prev), typeof(float(scan_sd))
    )
    out = Vector{T}(undef, m)
    @inbounds for i in 1:m
        out[i] = onset_report_scale(
            means[i], level_cur[i], level_prev[i], τ_cur[i], τ_prev[i];
            scan_sd
        )
    end
    return out
...
increment mean `μ` between two modelled cumulative levels `level_cur` and
`level_prev`, each read with its own snapshot's fitted digitisation-noise
scale (`τ_cur`, `τ_prev`; `0` for a read against the virtual empty
predecessor). The vector method calls this, so the two cannot drift apart.
The forecast ([`forecast_onsets`](@ref)) calls it directly to give a
projected reporting increment the same observation scale the likelihood
...
`grid_start` is `γ`'s own origin (the calendar walk's, which
[`onset_reporting_model`](@ref) may start later than the earliest scored
onset date) and `alpha_grid_start` is `alpha`'s (the ascertainment walk's,
always the earliest scored onset date). They default to the same value,
which is correct whenever the two coincide, but must be passed separately
once the calendar walk's own grid has been narrowed. Both spans start
after grid day 1 and end at or before `as_of`, so both the oldest and the
most recent terms need a calendar effect and an ascertainment level the
fit has no estimate for. [`onset_report_F`](@ref) already holds both flat
at their nearest fitted edge (see [`onset_report_G`](@ref)), so no
separate extrapolated form is needed here.
...
function onset_report_expected_total(
    ...
    @inbounds for u in 1:ge
        δ = as_of - u
        α = alpha[clamp(u - Int(alpha_grid_start) + 1, 1, na)]
...
smooth calendar-time effect on. Indexing this walk on onset date would
leave it unidentified against the reproduction-number walk, both free to
explain the same onset-date-indexed rise and fall. Report date lags onset
...
line-list re-analysis found. The wide SD leaves the per-delay random effect
free to depart substantially, so the fitted delay is correspondingly
uncertain (that analysis put its 7-day reporting fraction at 43-68%, wide
because the digitisation noise is comparable to the between-vintage
increments the estimate rests on).
...
counting variation and a fitted digitisation-noise scale `τ`, one per
surviving snapshot, carried on a weekly random walk over the report-date
span (`log_τ0 ~ Normal(log(3), 0.7)`, `σ_τ ~ truncated(Normal(0, 0.5);
lower = 0)`, the walk sampled centred like
[`background_walk_model`](@ref)'s). A cell's scale reads `τ` off both the
snapshot it belongs to and the one it corrects against, so a level cell
(differenced against the virtual empty predecessor) carries one snapshot's
noise and a genuine correction carries two. `τ`'s prior median of 3 counts
is the old fixed two-read pixel floor; the walk lets it drift with the
scans, which have grown noisier over the surveillance window.

**The scan error belongs to the figure, not to the bar.** A bar's height is
read in pixels and converted with the axis scale that scan calibrated, so
the digitisation error splits into an absolute per-bar term (the ≈2.1-case
pixel noise) and a multiplicative term that is one number for the whole
figure. `data/README.md` measures the second directly, as each vintage's
digitised total against the total the figure prints: -5.0% to +1.6% over
the audited vintages, one value per scan. Two mechanisms feed it. The axis
calibration is read per figure, and the digitiser's colour masks are fixed
thresholds against a render blur that varies with the embedded image's
size, so a blurrier scan loses more of every bar's edge. Neither explains
SitRep 088, whose digitised total falls 184 below SitRep 087 on the same
...
where the joint fit's divergent transitions concentrated. Scored as
independent per-cell noise instead (the per-cell scale carrying counting
variation and the fitted `τ` noise scale alone), a snapshot's cells can
only miss in uncorrelated directions, so the net correction is far too
tightly predicted (1 of 11 snapshots inside a nominal 50% interval) even
though the aggregate variance ratio is 1.07.

`σ_scan ~ scan_sd_prior` is estimated rather than fixed at the audited
spread, which rests on a handful of vintages, but its prior is a
half-normal centred to put that spread (an SD of about 2.5%) in its bulk,
unbounded above so a scan noisier than the audited handful is reachable
rather than excluded by construction.

The shared level also gives the fit somewhere to put a snapshot that
reprints nothing new. A vintage whose figure reads at the same level as its
predecessor scores increments of about zero across its whole row. Without a
per-scan level the only way to fit that row is to drive the reporting
hazard towards zero at the delays it covers, which propagates into every
other snapshot's delay shape.
...
grid_start, grid_end, alpha, alpha_grid_start, noise_scale, σ_τ, log_τ0,
σ_scan, η0, σ_h0, σ_γ, β, σ_a)` with `modelled` the per-cell increment means
the likelihood scores (each vintage's scan level applied), `unscanned` the
same means before it (the epidemiological signal alone), `scan_level` the
per-vintage multipliers, `grid_start` the report-date grid day the
calendar walk `γ` is indexed from (see above; not necessarily
`minimum(onset_days)`), `grid_end` the report-date grid day the calendar
walk was built up to (`max(report_days)`, or `grid_start` when the history
is empty), `alpha_grid_start` the onset-date grid day `alpha` is indexed
from (always `minimum(onset_days)`, or `1` when the history is empty; see
[`onset_report_expected_total`](@ref) for why this is kept separate from
`grid_start`), `noise_scale` the fitted per-snapshot digitisation-noise
scale (one entry per surviving vintage in vintage order), and the
...
@model function onset_reporting_model(
        ...
        D::Integer = ONSET_REPORT_MAX_DELAY,
        scan_sd_prior = truncated(Normal(0.0, 0.03); lower = 0.0),
        tau0_prior = Normal(log(3.0), 0.7),
        tau_walk_sigma_prior = truncated(Normal(0.0, 0.5); lower = 0.0),
        ν::Real = 4.0
    )
    ...
    m = length(onset_days)
    ## Onset-date grid origin: the earliest scored onset date, or 1 for an
    ## empty history. The delay-CDF table and the ascertainment walk both
    ## span `[u_lo, grid_end]`.
    ...
    cdf_table = onset_report_cdf_table(
        ...
        hazard_state.γ, hazard_state.grid_start, u_lo,
        grid_end
    )
    ...
    asc_state ~ to_submodel(
        ascertainment(anchor_series, u_lo, grid_end), false
    )
    alpha = asc_state.alpha

    ## Which cells share a scan (for the per-scan level error) and which
    ## share a noise-walk knot window (for `τ`, below): both are indexed
    ## by vintage, the sorted distinct report days of the scored cells.
    ...
    ## which carries counting variation and the fitted `τ` noise scale
    ## alone.
    ...
    ## Per-snapshot digitisation-noise scale `τ`, on a weekly random walk
    ## over the report-date span `[min(report_days), max(report_days)]`.
    ## Sampled centred (each step directly at `Normal(0, σ_τ)`, mirroring
    ## `background_walk_model`'s centred form): the triangle scores every
    ## printed cell, so the walk is strongly informed and the non-centred
    ## form would funnel. `log_τ0` anchors the walk's level at the earliest
    ## surviving vintage; `τ_steps` carries it from there.
    nt_r = m > 0 ?
        max(maximum(report_days) - minimum(report_days) + 1, 1) : 1
    tau_days = knot_days(nt_r; week = 7, start = 1)
    nb_r = length(tau_days)
    log_τ0 ~ tau0_prior
    σ_τ ~ tau_walk_sigma_prior
    τ_steps ~ product_distribution(
        fill(Normal(0, σ_τ + eps(typeof(float(σ_τ)))), max(nb_r - 1, 1))
    )
    log_τ_knots = vcat(
        log_τ0, log_τ0 .+ cumsum(τ_steps[1:max(nb_r - 1, 0)])
    )
    log_τ = interpolate_knots(log_τ_knots, tau_days, nt_r)
    ## One `τ` per surviving vintage, in the same sorted-report-day order
    ## `onset_vintage_indices` uses, then broadcast onto each cell's own
    ## current and previous read. A cell's previous read is the sentinel
    ## `0` for the virtual empty predecessor, which contributes a `τ_prev`
    ## of exactly zero rather than indexing out of bounds.
    min_report = m > 0 ? minimum(report_days) : 0
    report_day_list = m > 0 ? sort(unique(report_days)) : Int[]
    noise_scale = [
        exp(log_τ[rd - min_report + 1]) for rd in report_day_list
    ]
    τT = eltype(noise_scale)
    τ_cur = [noise_scale[vintages.vintage_idx[i]] for i in 1:m]
    τ_prev = [
        vintages.prev_vintage_idx[i] == 0 ? zero(τT) :
            noise_scale[vintages.prev_vintage_idx[i]]
            for i in 1:m
    ]
    ...
    scales = onset_report_scales(
        ...
        scanned.level_prev, τ_cur, τ_prev
    )
    ...
    return (;
        ...
        alpha_grid_start = u_lo,
        noise_scale, σ_τ, log_τ0,
...
ones: at every one of the 17 shared vintages, Ituri + Nord-Kivu + Sud-Kivu
equals the national confirmed count [`confirmed_cases_model`](@ref) already
scores. Fitting the per-province counts with their own count likelihood
would put the same observations into the joint density twice.

The information the spatial tables add is purely *spatial*, how the
confirmed cases divide between provinces, so the likelihood factorises as
...
with the last patch taking the remainder. `ρ` is an overdispersion shared
across provinces and vintages, absorbing the extra-Binomial variation in
how cases are attributed to provinces (reporting lags between the
provincial and national tables, reassignment of cases between health
zones). `ρ → 0` recovers a plain Multinomial split.
