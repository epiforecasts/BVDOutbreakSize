...
##
## The per-window composition `φ`, decaying severity upshift `δ_i` and the
## assay sensitivity/specificity transform to the tested-positive
## probability `p = s·q + (1 − spec)(1 − q)` are all applied here. The
## false-positive term `(1 − spec)(1 − q)` makes the confirmed counts
## respond to the non-BVD share `1 − q`, so the laboratory data identify
## the background `λ_bg` through `φ` rather than the BVD signal alone.
function composition_positivity(window_days, bvd_window, bg_window,
        c_window, δ0, dscale, s_t, sp_t, lo, hi)
...
@model function confirmed_cases_model(
    ...
    if positivity_link === :composition
        ...
        ## The composition φ, severity upshift and the assay
        ## sensitivity/specificity transform `p = s·q + (1 − spec)(1 − q)`
        ## all live in `composition_positivity` (a plain function, so its
        ## working variables are not boxed closure captures under Enzyme).
