...
# cache), so a build skips the compile. The Mooncake rules are keyed by method
# signature, not data size, so a 40-day synthetic fit compiles what the
# full-size fits reuse. The gradient path (not a NUTS fit) is the exact
