...
#
# AirspeedVelocity runs the suite; this turns what it wrote into the comment.
# Its own table is a flat list of every leaf with a median ratio, no neutral
# band, no threshold and no way to set one: the action has thirteen inputs
# and none of them is a comparison threshold. Two things this repository
# needs are therefore not in it.
#
