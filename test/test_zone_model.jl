...
@testitem "bvd_zone: the sampled dimension and the optional blocks" setup = [
    ...
    ## offset per zone, where the inputs carry the kernel blocks.
    zdm = zone_inputs(syn; zones = zone_metadata(syn)).model_data
    @test dimension(bvd_zone(zdm; mixing = true)) == dim + 2 + syn.nz
    ## Without them the model falls back to unmixed rather than gaining
    ## dimensions the renewal never reads.
    @test dimension(bvd_zone(zd; mixing = true)) == dim
