...
@testitem "dependent fits follow their parents in the registry" tags = [
    ...
    ## Every parent a spec names is listed before it. Only a dependent spec
    ## carries a `needs` field; a base spec reads back as an empty list.
    seen = String[]
    for s in specs
        @test fit_needs(s) isa Vector{String}
        for parent in fit_needs(s)
            @test parent in seen
        end
        push!(seen, s.id)
    end
    @test isempty(fit_needs((; id = "base", kind = :chain)))
    ...
    @test dependent == ["local", "local_frozen_validation"]
    ...
    ## The CI groups partition the same specs, and `zone` is exactly the
    ## dependent stage, so `list.jl --groups` and `BVD_FIT_STAGE` cannot
    ## disagree about which fits need a cached parent.
    groups = Dict(
        g => [s.id for s in specs if fit_group(s) == g]
            for g in FIT_GROUPS
    )
    @test groups["zone"] == dependent
    @test sort(reduce(vcat, values(groups))) == sort(
        fit_ids(
            obs; run_sensitivity = true
        )
    )
    @test groups["joint"] == ["joint", "sens_no_patches"]
    @test "frozen_validation" in groups["frozen"]
    @test all(startswith("sens_"), groups["sensitivity"])
end
