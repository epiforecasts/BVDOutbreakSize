...
@testsnippet ADRuleCases begin
    ...
    function rule_cases(rng)
        ...
        ## A population far above the epidemic, as in the model, and one the
        ## epidemic uses up, so the susceptible terms of the adjoint carry
        ## weight.
        add!(
        ...
        add!(
            "depletion binds", renewal_infections, Rt(40) .+ 1, pmf(rng, 12),
            rand(rng, 7) .+ 1, 200.0
        )
        add!(
        ...
        add!(
            "3 patches, depletion binds", patch_infections,
            patch_args(3, 40, 7, 12; N = 100.0)...
        )
        add!("1 patch", patch_infections, patch_args(1, 30, 5, 10; N = 50.0)...)
        add!("G > n", patch_infections, patch_args(3, 10, 3, 15; N = 50.0)...)
        add!(
        ...
        add!(
            ...
            patch_args(3, 220, 14, 35; N = 1.0e7)...; perf = true
        )
...
@testitem "Mooncake rules: pullbacks leave the output tangent as given" tags = [
    ...
    args = (
        ...
        rand(rng, np, n) .* 0.5, [300.0, 400.0, 500.0],
    )
    out, pb = Mooncake.rrule!!(
