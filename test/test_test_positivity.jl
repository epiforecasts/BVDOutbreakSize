...
@testitem "confirmed ramp Δλ=0 reproduces constant" tags=[:slow] begin
    ## A confirmed_cases_model built with a Δλ=0 ramp must give identical
    ## per-edge positivity to one built with the constant closed-form
    ## background, for matched (λ0, τ, delays, growth).
    using Turing: sample, Prior, @model, to_submodel
    import FlexiChains
