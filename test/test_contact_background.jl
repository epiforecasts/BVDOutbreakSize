...
@testitem "contact covariate enters the joint and adds a sampled coefficient" tags=[:slow] begin
    using Turing: sample, Prior
    import FlexiChains
    using BVDOutbreakSize: load_observations, bvd_joint, expand_covariate,
                           genetic_seeding_model

    obs = load_observations()
    breakpoint = obs.n - obs.who_first_sitrep_days
    cov = expand_covariate(obs.contact_followup_history.days,
        obs.contact_followup_history.values, obs.n)

    make(cc) = bvd_joint(obs.n, obs.exported_cases, obs.total_deaths,
        ...
        background_re = true,
        contact_covariate = cc,
        genetic = genetic_seeding_model,
        tmrca_days = obs.tmrca_days)

    ## With the covariate the joint samples the contact coefficient and stays
    ## finite and positive.
    chn = sample(make(cov), Prior(), 30;
        chain_type = FlexiChains.VNChain, progress = false)
    ks = string.(collect(keys(chn)))
    @test any(k -> occursin("contact_state", k) || occursin("β_contact", k), ks)
    C_T = vec(Array(chn[:C_T]))
    @test length(C_T) == 30
    @test all(isfinite, C_T)
    @test all(C_T .> 0)

    ## Without the covariate (the headline default) no contact coefficient is
    ## sampled, so the fit is unchanged from the covariate-free model.
    chn0 = sample(make(nothing), Prior(), 30;
        chain_type = FlexiChains.VNChain, progress = false)
    ks0 = string.(collect(keys(chn0)))
    @test !any(k -> occursin("contact_state", k) || occursin("β_contact", k), ks0)
end
