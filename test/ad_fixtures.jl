...
function backends()
    ...
    try
        ...
        println(
            stderr, "[fixtures] no Enzyme backend: ",
...
function latent_draw(; n::Integer = N, seed::Integer = SEED)
    m = BVDOutbreakSize._latent(
        n, missing, infection_model,
...
const PATCH_KERNEL_ON = [0.0 0.0 0.0; 1.0e-4 0.0 0.0; 1.0e-5 0.0 0.0]
...
function scenarios(; n::Integer = N, joint::Bool = false)
    ...
    push!(
        out,
        Scenario(
            "latent (infection + onset staging)", "Latent",
            BVDOutbreakSize._latent(
                n, missing, infection_model,
                onset_incidence_model
            ), SEED
        )
    ...
    push!(
        out,
        Scenario(
            "patch_infection_model (uncoupled)", "Latent",
            patch_infection_model(
                60, 3;
                importation_kernel = PATCH_KERNEL_OFF
            ), SEED
        )
    )
    push!(
        out,
        Scenario(
            "patch_infection_model (coupled)", "Latent",
            patch_infection_model(
                60, 3;
    ...
    push!(
        out,
        Scenario(
            "treatment_flow_model", "Submodel",
            treatment_flow_model(
                ISOLATION_HISTORY, cases.bvd_reports_daily,
    ...
    push!(
        out,
        Scenario(
            "onset_reporting_model", "Submodel",
    ...
    push!(
        out,
        Scenario(
            "province_composition_model", "Submodel",
