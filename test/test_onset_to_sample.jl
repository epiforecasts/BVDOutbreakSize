...
@testitem "nejm_onset_to_sample returns the cohort summaries and SEs" begin
    ...
    @test isapprox(cfg.mean_se, (13.5 - 5.3) / 2 / 1.96; atol = 1.0e-9)
    @test isapprox(cfg.median_se, (7.84 - 3.46) / 2 / 1.96; atol = 1.0e-9)
end
...
@testitem "onset_to_sample_logweight peaks at the reported summaries" begin
    ...
    @test isapprox(
        gamma_median_wh(cfg.mean_obs, sd_star), cfg.median_obs;
        atol = 1.0e-6
    )
    ...
    at_target = onset_to_sample_logweight(
        rep_m, sqrt(rep_v), rec_m,
        sqrt(rec_v), cfg
    )
    ## Perturbing either leg away from the matched mean lowers the log-weight.
    off_mean = onset_to_sample_logweight(
        rep_m + 2.0, sqrt(rep_v), rec_m,
        sqrt(rec_v), cfg
    )
    off_spread = onset_to_sample_logweight(
        rep_m, sqrt(rep_v * 3), rec_m,
        sqrt(rec_v * 3), cfg
    )
    @test at_target > off_mean
    @test at_target > off_spread
    @test at_target ≈ 0 atol = 1.0e-6      # both residuals zero at the target
end
