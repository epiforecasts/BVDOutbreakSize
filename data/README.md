...
Late reporting only ever adds cases, so the TOTAL is non-decreasing — but an
individual bar can legitimately fall, because onset dates get corrected and a
re-dated case moves one bar down and a nearby bar up.
Testing whether a fall is paired with a rise within two days separates the two:
065 → 066, 067 → 068 and 069 → 072 lose a handful of cases with every fall
paired, which is date correction, whereas 064 → 065 loses 39 across 35 settled
days with 3% paired and 068 → 069 loses 100 across 36 days.
What identifies scan error is therefore a large fall with nothing nearby
absorbing it, not a fall as such.
A between-vintage increment of a few cases is at or below that noise floor,
which bounds what a reporting-delay estimate built on those increments can
support.
Note that 064 → 065 is also the one vintage with a documented downward
revision — SitRep 065's harmonisation moved 6 cases out of Haut-Uélé — so a
...
ids, so a new reprint is caught without a code change; treating a reprint as a
fresh snapshot would fabricate an increment of exactly zero and bias the fitted
delay towards fast reporting.
...
SitRep 068 (21 July) is included: its PDF was unreachable while the INSP fetch
was broken, and it does carry a figure (n = 2 308).
