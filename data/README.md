...
SitReps 101 and 102 (23/24 August) both keep the 0/25/50/75 y-axis grid. SitRep 101's rightmost tick advances to 24 August - the first movement in seven vintages off the 094-100 plateau's fixed 17 August - read directly off the rendered image (`n = 4 287`) rather than inferred; it is one day ahead of 101's own 23 August rapportage date, matching the `date de publication` pattern already documented for SitReps 068/082/094. Digitises to 4084 (Julia), -4.7% against the printed n, inside the established band. The 100 -> 101 L1 date-alignment test is cleanly minimised at shift 0 (124 against 803 at -1 and 819 at +1), confirming the tick reading. SitRep 102's rightmost tick is ALSO 24 August (`n = 4 372`) despite 102's own rapportage date being 24 August itself (no one-day lag this time) - read directly off a distinct embedded image (different md5 and render size, 825x455 against 101's 827x534, so not a reprint), confirming the axis is anchored to a fixed weekly grid rather than always advancing with the report date, the same mechanism already seen across the 094-100 and other plateaus. Digitises to 4024, -8.0% against the printed n, a couple of points past the previous -5.0% floor (SitReps 079/080) rather than an order of magnitude off; per section 4b the neighbours were checked before accepting it: the stable region (onset dates >=3 weeks before each vintage's own report date) reads 3236 (099) -> 3335 (100) -> 3375 (101) -> 3304 (102), a fall of 71 (-2.1%) from 101 to 102 that is within the two-sided scatter this stream already documents (contrast the 098 exclusion, where the stable-region gap against BOTH neighbours was ~280 cases, two orders of magnitude larger), and the embedded image size (825x455) is consistent with every recent neighbour (097 832x496, 099 830x517, 100 828x508, 101 827x534) rather than 098's outlier 1267x789. The 101 -> 102 L1 date-alignment test is cleanly minimised at shift 0 (366 against 604 at -1 and 994 at +1). So 102's scan is recorded as extending the known range, not rejected. Both vintages' Julia and Python digitiser output is byte-identical for every 101/102 row (checked directly), so the cross-implementation divergence documented above does not extend to these two vintages. Both scripts were verified to reproduce every previously committed block (059-100) unchanged before the 101/102 blocks were accepted.
